...
function setNuiActive(booleanOrNil)
  ...
  if boolean ~= isNuiActive then
    if boolean then TriggerServerEvent('datasilta:request-data') end
    isNuiActive = boolean
...
CreateThread(function()
  if isNuiActive then TriggerServerEvent('datasilta:request-data') end
  Wait(300000)
end)
...
RegisterNetEvent('datasilta:update-resource-data')
AddEventHandler('datasilta:update-resource-data', function (resourceData)
...
RegisterNetEvent('datasilta:update-time-data')
AddEventHandler('datasilta:update-time-data', function (timeData)
  local timeArray = {}
...
RegisterNetEvent('datasilta:update-slow-queries')
AddEventHandler('datasilta:update-slow-queries', function(slowQueryData)
...
RegisterNetEvent('datasilta:update-status')
AddEventHandler('datasilta:update-status', function(statusData)
  SendNUIMessage({
    type = 'onStatusData',
    status = statusData,
  });
...
RegisterNetEvent('datasilta:update-variables')
AddEventHandler('datasilta:update-variables', function(variableData)
  SendNUIMessage({
    type = 'onVariableData',
...
RegisterNUICallback('request-server-status', function()
  TriggerServerEvent('datasilta:request-server-status')
end)

RegisterNetEvent('datasilta:get-table-schema')
AddEventHandler('datasilta:get-table-schema', function(tableName, schema)
  SendNUIMessage({
    type = 'onTableSchema',
    tableName = tableName,
    schema = schema,
...
RegisterNUICallback('request-table-schema', function(tableName)
  TriggerServerEvent('datasilta:request-table-schema', tableName)
end)