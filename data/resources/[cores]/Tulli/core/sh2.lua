...
_Queue.JoinCbs = {}
_Queue.TempPriority = {}
_Queue.Jonotusaika = GetGameTimer() + Asetus.Jonotusaika and Asetus.Jonotusaika or 0
...
Queue.InitHostName = Queue.InitHostName ~= "RevohkaRP" and Queue.InitHostName or false

for id, power in pairs(Asetus.Priority) do
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] on sijalla prioriteetti %d/%d jonossa", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] on sijalla %d/%d jonossa", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        done(Asetus.Language.connectingerr)
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    Citizen.CreateThread(function()
        while connecting do
            ...
            deferrals.update(Asetus.Language.connecting)
        end
    end)
    Citizen.Wait(500)
    local function done(msg, _deferrals)
        ...
        if not msg then
            deferrals.done()
            if Asetus.Armo then Queue:AddPriority(ids[1], Asetus.ArmoKerrat, Asetus.GraceTime) end
        else
    ...
    if not ids then
        done(Asetus.Language.iderr)
        CancelEvent()
        Queue:DebugPrint("Dropped " .. name .. ", couldn't retrieve any of their id's")
        return
    end
    if Asetus.Identiteetti and not Queue:IsSteamRunning(src) then
        done(Asetus.Language.steam)
        CancelEvent()
        return
    end
    local allow
    ...
    if not allow then return end
    if Asetus.Prioriteetti and not Queue:IsPriority(ids) then done(Asetus.Language.wlonly) return end
    ...
    if not pos or not data then
        done(Asetus.Language.err .. " [1]")
        Queue:RemoveFromQueue(ids)
        Queue:RemoveFromConnecting(ids)
        CancelEvent()
        return
    end
    if Queue:NotFull(true) and _Queue.Jonotusaika <= GetGameTimer() then
        local added = Queue:AddToConnecting(ids, true, true, done)
        if not added then CancelEvent() return end
        done()
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] on tulossa kaupunkiin")
        return
    end
    update(string_format(Asetus.Language.pos .. ((Queue:TempSize() and Asetus.Tilanne) and " (" .. Queue:TempSize() .. " temp)" or "00:00:00"), pos, Queue:GetSize(), ""))
    ...
    while true do
        ...
        if data.timeout >= Asetus.Jonotusaika and os_time() - connectTime > 5 then
            remove("[Queue] Removed due to timeout")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] was removed from the queue because they timed out")
            return
        end
        if pos <= 1 and Queue:NotFull() and _Queue.Jonotusaika <= GetGameTimer() then
            local added = Queue:AddToConnecting(ids)
            update(Asetus.Language.joining, data.deferrals)
            Citizen.Wait(500)
            if not added then
                done(Asetus.Language.connectingerr)
                CancelEvent()
        ...
        local msg = string_format(Asetus.Language.pos .. ((Queue:TempSize() and Asetus.Tilanne) and " (" .. Queue:TempSize() .. " temp)" or ""), pos, Queue:GetSize(), qTime)
        update(msg, data.deferrals)
    end
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Asetus.Aikakatkaisu) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
