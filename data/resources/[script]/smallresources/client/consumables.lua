...
AddEventHandler("consumables:client:UseJoint", function()
    rg.toiminnot.Progressbar("smoke_joint", "A dar umas passas..", 1500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["joint"], "remove")
...
AddEventHandler("consumables:client:UseParachute", function()
    EquipParachuteAnim()
    rg.toiminnot.Progressbar("use_parachute", "parachute using..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["parachute"], "remove")
...
AddEventHandler("consumables:client:ResetParachute", function()
    if ParachuteEquiped then 
        EquipParachuteAnim()
        rg.toiminnot.Progressbar("reset_parachute", "A abrir o paraquedas..", 40000, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            local ped = GetPlayerPed(-1)
            TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["parachute"], "add")
        ...
        rg.toiminnot.Notify("Não tens nenhum paraquedas aberto!", "error")
    end
end)
...
--         TriggerEvent("inventory:client:Itembox", rg.datapankki.Items["smoketrailred"], "remove")
--     else
--         rg.toiminnot.Notify("You need to have a paracute to activate smoke!", "error")    
--     end
-- end)

RegisterNetEvent("consumables:client:UseArmor")
AddEventHandler("consumables:client:UseArmor", function()
    rg.toiminnot.Progressbar("use_armor", "A vestir o Colete..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["armor"], "remove")
        TriggerServerEvent('hospital:server:SetArmor', 75)
        TriggerServerEvent("rg:Server:RemoveItem", "armor", 1)
...
AddEventHandler("consumables:client:UseHeavyArmor", function()
    ...
    rg.toiminnot.Progressbar("use_heavyarmor", "A vestir o Colete..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["heavyarmor"], "remove")
        TriggerServerEvent("rg:Server:RemoveItem", "heavyarmor", 1)
...
AddEventHandler("consumables:client:ResetArmor", function()
    ...
    if currentVest ~= nil and currentVestTexture ~= nil then 
        rg.toiminnot.Progressbar("remove_armor", "A remover o Colete..", 2500, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            SetPedArmour(ped, 0)
            TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["heavyarmor"], "add")
            TriggerServerEvent("rg:Server:AddItem", "heavyarmor", 1)
        end)
    else
        rg.toiminnot.Notify("Não estas a usar um colete...", "error")
    end
end)
...
AddEventHandler("consumables:client:DrinkAlcohol", function(itemName)
    ...
    rg.toiminnot.Progressbar("snort_coke", "Consumindo..", math.random(3000, 6000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items[itemName], "remove")
        TriggerServerEvent("rg:Server:RemoveItem", itemName, 1)
        TriggerServerEvent("rg:Server:SetMetaData", "thirst", rg.toiminnot.GetPlayerData().metadata["thirst"] + Consumeables[itemName])
        alcoholCount = alcoholCount + 1
        ...
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        rg.toiminnot.Notify("Cancelado..", "error")
    end)
end)

RegisterNetEvent("consumables:client:Cokebaggy")
AddEventHandler("consumables:client:Cokebaggy", function()
    rg.toiminnot.Progressbar("snort_coke", "Consumindo..", math.random(5000, 8000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerServerEvent("rg:Server:RemoveItem", "cokebaggy", 1)
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["cokebaggy"], "remove")
...
AddEventHandler("consumables:client:Crackbaggy", function()
    rg.toiminnot.Progressbar("snort_coke", "Smoking crack..", math.random(7000, 10000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        StopAnimTask(GetPlayerPed(-1), "switch@trevor@trev_smoking_meth", "trev_smoking_meth_loop", 1.0)
        TriggerServerEvent("rg:Server:RemoveItem", "crack_baggy", 1)
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["crack_baggy"], "remove")
...
AddEventHandler('consumables:client:EcstasyBaggy', function()
    rg.toiminnot.Progressbar("use_ecstasy", "Pillen poppen", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("rg:Server:RemoveItem", "xtcbaggy", 1)
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["xtcbaggy"], "remove")
        EcstasyEffect()
    end, function() -- Cancel
        StopAnimTask(GetPlayerPed(-1), "mp_suicide", "pill", 1.0)
        rg.toiminnot.Notify("Cancelado", "error")
...
AddEventHandler("consumables:client:Eat", function(itemName)
    ...
    rg.toiminnot.Progressbar("eat_something", "A Comer..", 2500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items[itemName], "remove")
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        TriggerServerEvent("rg:Server:SetMetaData", "hunger", rg.toiminnot.GetPlayerData().metadata["hunger"] + Consumeables[itemName])
...
AddEventHandler("consumables:client:Drink", function(itemName)
    TriggerEvent('animations:client:EmoteCommandStart', {"drink"})
    rg.toiminnot.Progressbar("drink_something", "A Beber..", 2500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
