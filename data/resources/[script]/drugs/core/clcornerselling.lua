...
AddEventHandler('drugs:client:cornerselling', function(data)
    rg.toiminnot.HuutoKaiku('drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            availableDrugs = result

            if not cornerselling then
                cornerselling = true
                rg.toiminnot.Notify('You are now selling drugs...')
                startLocation = GetEntityCoords(GetPlayerPed(-1))
                TaskStartScenarioInPlace(GetPlayerPed(-1), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
            else
                cornerselling = false
                rg.toiminnot.Notify('You stopped selling drugs...')
                ClearPedTasks(GetPlayerPed(-1))
            end
        else
            rg.toiminnot.Notify('You dont have drugs with you...', 'error')
        end
    end)
end)

function toFarAway()
    rg.toiminnot.Notify('Leave the place, start over!', 'error')
    cornerselling = false
    hasTarget = false
    busySelling = false
    startLocation = nil
        ...
        if stealingPed ~= nil and stealData ~= nil then
            if IsEntityDead(stealingPed) then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, pedpos.x, pedpos.y, pedpos.z, true) < 1.5) then
                    DrawText3D(pedpos.x, pedpos.y, pedpos.z, "[E] Take")
                    if IsControlJustReleased(0, Keys["E"]) then
                        ...
                        Citizen.Wait(2000)
                        ClearPedTasks(GetPlayerPed(-1))
                        TriggerServerEvent("rg:Server:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', rg.datapankki.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                rg.toiminnot.Notify('You were robbed and you lost '..bagAmount..' bag(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 then
                    rg.toiminnot.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' for $'..randomPrice..'? / ~g~G~w~ Decline offer')
                    if IsControlJustPressed(0, Keys["E"]) then
                        rg.toiminnot.Notify('Oferta aceite!', 'success')
                    ...
                    if IsControlJustPressed(0, Keys["G"]) then
                        rg.toiminnot.Notify('Canceled offer!', 'error')
                        hasTarget = false
