...
rg.toiminnot.CreateCallback("weapons:server:GetConfig", function(source, cb)
...
--     rg.toiminnot.datasilta(false, "SELECT * FROM `playerammo` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
...
rg.toiminnot.CreateCallback("weapon:server:GetWeaponAmmo", function(source, cb, WeaponData)
...
rg.toiminnot.CreateUseableItem("pistol_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_PISTOL", 12, item)
end)

rg.toiminnot.CreateUseableItem("sniper_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "SNIPER_PISTOL", 5, item)
end)

rg.toiminnot.CreateUseableItem("rifle_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_RIFLE", 30, item)
end)

rg.toiminnot.CreateUseableItem("smg_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SMG", 20, item)
end)

rg.toiminnot.CreateUseableItem("shotgun_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SHOTGUN", 10, item)
end)

rg.toiminnot.CreateUseableItem("mg_ammo", function(source, item)
...
--     local src = source
--     local Player = rg.toiminnot.GetPlayer(src)
--     local WeaponData = rg.datapankki.Weapons[GetHashKey(data.name)]
...
--                 TriggerClientEvent('rg:Ilmoitus', src, "Your weapon is broken, u need to repair it before u can use it again.", "error")
--             end
--         else
            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        WeaponSlot.info.quality = 0
                        TriggerClientEvent('inventory:client:UseWeapon', src, data)
                        TriggerClientEvent('rg:Ilmoitus', src, "Sua arma está quebrada, você precisa consertá-la antes de usá-la novamente.", "error")
                        break
                    end
                end
            else
...
rg.komennot.Add("repairweapon", "Comando de reparo de arma para equipe", {{name="hp", help="HP da sua arma"}}, true, function(source, args)
...
rg.toiminnot.CreateCallback("weapons:server:RepairWeapon", function(source, cb, RepairPoint, data)
    local src = source
    local Player = rg.toiminnot.GetPlayer(src)
    local minute = 60 * 1000
    local Timeout = math.random(5 * minute, 10 * minute)
    local WeaponData = rg.datapankki.Weapons[GetHashKey(data.name)]
    local WeaponClass = (rg.datapankki.SplitStr(WeaponData.ammotype, "_")[2]):lower()
    ...
    if Player.PlayerData.items[data.slot] ~= nil then
        if Player.PlayerData.items[data.slot].info.quality ~= nil then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCotsts[WeaponClass]) then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items[data.name], "remove")
                ...
                TriggerClientEvent("rg:Ilmoitus", src, "Esta arma não está danificada..", "error")
                cb(false)
            end
        else
            TriggerClientEvent("rg:Ilmoitus", src, "Esta arma não está danificada..", "error")
        ...
        TriggerClientEvent('rg:Ilmoitus', src, "Você não tinha uma arma em suas mãos..", "error")
...
AddEventHandler("weapons:server:TakeBackWeapon", function(k, data)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items[itemdata.name], "add")
    Config.WeaponRepairPoints[k].IsRepairing = false
    Config.WeaponRepairPoints[k].RepairingData = {}
    TriggerClientEvent('weapons:client:SyncRepairShops', -1, Config.WeaponRepairPoints[k], k)
end)

rg.toiminnot.CreateUseableItem("pistol_suppressor", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

rg.toiminnot.CreateUseableItem("smg_suppressor", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

rg.toiminnot.CreateUseableItem("rifle_suppressor", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

rg.toiminnot.CreateUseableItem("pistol_extendedclip", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

rg.toiminnot.CreateUseableItem("smg_extendedclip", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

rg.toiminnot.CreateUseableItem("smg_flashlight", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "flashlight")
end)

rg.toiminnot.CreateUseableItem("smg_scope", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
...
rg.toiminnot.CreateUseableItem("rifle_extendedclip", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

rg.toiminnot.CreateUseableItem("rifle_drummag", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("rg:Ilmoitus", src, "Você já tem "..AttachmentData.label:lower().." na sua arma.", "error", 3500)
            end
        else
...
rg.toiminnot.CreateCallback('weapons:server:RemoveAttachment', function(source, cb, AttachmentData, ItemData)
    local src = source
    local Player = rg.toiminnot.GetPlayer(src)
    local Inventory = Player.PlayerData.items
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items[AttachmentComponent.item], "add")
                TriggerClientEvent("rg:Ilmoitus", src, "Você removeu "..AttachmentComponent.label.." da sua arma!", "error")
