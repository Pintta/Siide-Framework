...
Citizen.CreateThread(function()
	for id, scrapyard in pairs(Config.Locations) do
        ...
        AddTextComponentSubstringPlayerName("Purkaamo")
							...
							if IsControlJustReleased(0, Keys["E"]) then
								if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										rg.toiminnot.Notify("This vehicle cannot be scrapped..", "error")
									end
								else
									rg.toiminnot.Notify("You are not the driver..", "error")
								end
							end
						end
					end
...
RegisterNetEvent('scapyard:client:setNewVehicles')
AddEventHandler('scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = rg.datapankki.Vehicles[v]
		...
		rg.toiminnot.Notify("You are not allowed to demolish vehicles now..", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	rg.toiminnot.Progressbar("scrap_vehicle", "Voertuig slopen..", scrapTime, false, true, {
