...
Citizen.CreateThread(function()
    for k, v in pairs(Config.RentalPoints) do
        ...
        SetBlipColour(RentalPoints, 81)

        BeginTextCommandSetBlipName("STRING")
        AddTextComponentSubstringPlayerName("Vuokraamo - Noutopiste")
    ...
    for k, v in pairs(Config.DeliveryPoints) do
        ...
        SetBlipColour(DeliveryPoints, 81)

        BeginTextCommandSetBlipName("STRING")
        AddTextComponentSubstringPlayerName("Vuokraamo - Palautuspiste")
...
RentVehicle = function(selectedVehicle)
    ...
    if IsPedInAnyVehicle(ped) then
        rg.toiminnot.Notify('this can only when not in a vehicle..', 'error')
        return
    end

    local vehiclePlate = "VUOKRA"..math.random(1, 100)
    TriggerServerEvent('vehiclerental:server:SetVehicleRented', vehiclePlate, true, Config.RentalVehicles[selectedVehicle])

    RentedVehicleData = selectedVehicle
end

RegisterNetEvent('vehiclerental:server:SpawnRentedVehicle')
AddEventHandler('vehiclerental:server:SpawnRentedVehicle', function(vehiclePlate, vehicleData)
    local ped = GetPlayerPed(-1)
    local coords = {
...
ReturnVehicle = function()
    ...
    rg.toiminnot.Notify('You do not have a deposit open..', 'error')
end
...
AcceptReturn = function()
    ...
    if noSpace(VehiclePlate) ~= noSpace(RentedVehiclePlate) then
        rg.toiminnot.Notify('This is not a rental vehicle..', 'error')
    else
        TriggerServerEvent('vehiclerental:server:SetVehicleRented', RentedVehiclePlate, false, Config.RentalVehicles[RentedVehicleData])
        rg.toiminnot.DeleteVehicle(CurrentVehicle)
