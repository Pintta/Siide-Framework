...
Citizen.CreateThread(function()
    while true do
        ...
        if rg ~= nil then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), true), -1) == GetPlayerPed(-1) then
                ...
                if LastVehicle ~= GetVehiclePedIsIn(GetPlayerPed(-1), false) then
                    rg.toiminnot.HuutoKaiku('vehiclekeys:CheckHasKey', function(result)
                        if result then
                            HasKey = true
        ...
        if not HasKey and IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and rg ~= nil and not IsHotwiring then
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(7)
        if not IsRobbing and isLoggedIn and rg ~= nil then
            if GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= nil and GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= 0 then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) then
                    if IsEntityDead(driver) then
                        IsRobbing = true
                        rg.toiminnot.Progressbar("rob_keys", "ANASTAT..", 3000, false, true, {}, {}, {}, {}, function()
            ...
            if rg.toiminnot.GetPlayerData().job.name ~= "police" then
...
RegisterNetEvent('vehiclekeys:client:AsetaOmistaja')
AddEventHandler('vehiclekeys:client:AsetaOmistaja', function(plate)
    ...
    HasKey = true
    rg.toiminnot.Notify('You picked the keys of the vehicle', 'success', 3500)
end)
...
function LockVehicle()
    local veh = rg.toiminnot.GetClosestVehicle()
    ...
    if veh ~= nil and GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 7.5 then
        rg.toiminnot.HuutoKaiku('vehiclekeys:CheckHasKey', function(result)
            if result then
                if HasKey then
                    ...
                    if vehLockStatus == 1 then
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 2)then
                            rg.toiminnot.Notify("Lukossa")
                        else
                            rg.toiminnot.Notify("No voi vittu..")
                        end
                    else
                        Citizen.Wait(750)
                        ClearPedTasks(GetPlayerPed(-1))
                ...
                rg.toiminnot.Notify('Sinulla ei ole ajoneuvon avaimia..', 'error')
            end
        end, plate)
    end
end
    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 1.5 then
            ...
            if (vehLockStatus > 1) then
                ...
                rg.toiminnot.Progressbar("lockpick_vehicledoor", "MURRAT..", lockpickTime, false, true, {
...
function Hotwire()
    if not HasKey then 
        ...
        PoliceCall()
        rg.toiminnot.Progressbar("hotwire_vehicle", "MURRAT", hotwireTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        ...
        if math.random(1, 100) <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    if rg.datapankki.VehicleModels[modelName] ~= nil then
                        Name = rg.datapankki.Vehicles[rg.datapankki.VehicleModels[modelName]["model"]]["brand"] .. ' ' .. rg.datapankki.Vehicles[rg.datapankki.VehicleModels[modelName]["model"]]["name"]
                    else
                        Name = "HÄTÄKESKUS"
                    end
