...
rg.toiminnot.CreateCallback('vehiclekeys:CheckHasKey', function(source, cb, plate)
...
AddEventHandler('vehiclekeys:server:GiveVehicleKeys', function(plate, target)
    ...
    if CheckOwner(plate, Player.PlayerData.citizenid) then
        if rg.toiminnot.GetPlayer(target) ~= nil then
            TriggerClientEvent('vehiclekeys:client:AsetaOmistaja', target, plate)
            TriggerClientEvent('rg:Ilmoitus', src, "You gave the keys!")
            TriggerClientEvent('rg:Ilmoitus', target, "You got the keys!")
        else
...
rg.komennot.Add("motor", "Toggle engine On/Off of the vehicle", {}, false, function(source, args)
	TriggerClientEvent('vehiclekeys:client:ToggleEngine', source)
end)

rg.komennot.Add("geefsleutels", "Give keys of the vehicle", {{name = "id", help = "Speler id"}}, true, function(source, args)
	local src = source
...
rg.toiminnot.CreateUseableItem("lockpick", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
