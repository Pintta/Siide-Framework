...
AddEventHandler('lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        rg.toiminnot.Notify('Ja estas a fazer uma corrida.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        rg.toiminnot.Notify('You have not yet set a checkpoint ..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        rg.toiminnot.Notify('The minimum of Checkpoints is 10', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 51.0 then
                        ...
                        rg.toiminnot.Notify('You can not do more than 50')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        rg.toiminnot.Notify('You have to have at least 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    rg.toiminnot.Notify('Click [9] again to confirm', 'error', 5000)
                else
                    ...
                    rg.toiminnot.Notify('Race Edit Canceled!', 'error')
...
function SaveRace()
    ...
    rg.toiminnot.Notify('Race: '..CreatorData.RaceName..' save!', 'success')
...
function AddCheckpoint()
    ...
    for id, CheckpointData in pairs(CreatorData.Checkpoints) do
        ...
        SetBlipColour(CheckpointData.blip, 81)
        ShowNumberOnBlip(CheckpointData.blip, id)
        SetBlipShowCone(CheckpointData.blip, false)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentSubstringPlayerName("Tarkastuspiste: "..id)
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            rg.toiminnot.Notify('You can not go so fast...', 'error')
        end
    else
        rg.toiminnot.Notify('You can not go so fast...', 'error')
    end
...
function SetupRace(RaceData, Laps)
    ...
    CurrentRaceData = {RaceId = RaceData.RaceId, Creator = RaceData.Creator, RaceName = RaceData.RaceName, Checkpoints = RaceData.Checkpoints, Started = false, CurrentCheckpoint = 1, TotalLaps = Laps, Lap = 1, RaceTime = 0, TotalTime = 0, BestLap = 0, Racers = {}}
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        ClearAreaOfObjects(v.offset.left.x, v.offset.left.y, v.offset.left.z, 50.0, 0)
        ...
        SetBlipColour(CurrentRaceData.Checkpoints[k].blip, 81)
        ShowNumberOnBlip(CurrentRaceData.Checkpoints[k].blip, k)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentSubstringPlayerName("Tarkastuspiste: "..k)
...
function DoPilePfx()
    if CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint] ~= nil then
        local Timeout = 500
        local Size = 2.0
        local left = showNonLoopParticle('Tulli', 'ent_sht_flame', CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint].offset.left, Size)
        local right = showNonLoopParticle('Tulli', 'ent_sht_flame', CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint].offset.right, Size)
...
AddEventHandler('lapraces:client:RaceCountdown', function()
    ...
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    rg.toiminnot.Notify('The race will start in 10 seconds', 'error', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    rg.toiminnot.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            rg.toiminnot.Notify('GO!', 'success', 1000)
        ...
        rg.toiminnot.Notify('You are currently notin a race..', 'error')
    end
end)
...
function FinishRace()
    ...
    if CurrentRaceData.BestLap ~= 0 then
        rg.toiminnot.Notify('Race finished '..SecondsToClock(CurrentRaceData.TotalTime)..', Whit best lap as: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        rg.toiminnot.Notify('Race finished '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
AddEventHandler('lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            rg.toiminnot.Notify(FinisherData.PlayerData.charinfo.firstname..' Finished on place : '..Place, 'error', 3500)
        end
    end
end)
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            rg.toiminnot.Notify('Go back to the start in 10 seconds: '..ToFarCountdown..'s', 'error', 500)
                        else
