...
AddEventHandler('rg:Client:OnPlayerLoaded', function()
    rg.toiminnot.GetPlayerData(function(PlayerData)
        PlayerJob = PlayerData.job
        if PlayerData.job.onduty then
            if PlayerData.job.name == "mechanic" then
                TriggerServerEvent("rg:ToggleDuty")
            end
        end
    end)
    isLoggedIn = true
    rg.toiminnot.HuutoKaiku('vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    rg.toiminnot.HuutoKaiku('vehicletuning:server:GetDrivingDistances', function(retval)
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Motonet")
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    rg.toiminnot.DeleteVehicle(GetVehiclePedIsIn(GetPlayerPed(-1)))
                                end
                            else
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            rg.toiminnot.Notify("Não podes colocar uma bicicleta na plataforma!", "error")
                                        end
                                    end
                                end
                            end
...
function OpenMenu()
    ...
    Menu.addButton("SULJE", "CloseMenu", nil) 
end
...
function SpawnListVehicle(model)
    ...
    rg.toiminnot.HaeAjoneuvo(model, function(veh)
...
function PartsMenu()
    ...
    Menu.addButton("TAKAISIN", "VehicleOptions", nil) 
    Menu.addButton("SULJE", "CloseMenu", nil) 
end
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Reparar ("..rg.datapankki.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("TAKAISIN", "VehicleOptions", nil)
    Menu.addButton("SULJE", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    rg.toiminnot.HuutoKaiku('inventory:server:GetStashItems', function(StashItems)
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    rg.toiminnot.Progressbar("repair_part", Config.ValuesLabels[part].." KORJAAT", math.random(5000, 10000), false, true, {
                    ...
                    rg.toiminnot.Notify('Não tens materiais suficientes no armario..', 'error')
                end
                break
            else
                rg.toiminnot.Notify('Não tens materiais suficientes no armario..', 'error')
...
AddEventHandler('vehicletuning:client:RepaireeePart', function(part)
    ...
    rg.toiminnot.Notify("De "..Config.ValuesLabels[part].." foi reparado!")
end)
    ...
    while true do 
        ...
        if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
            ...
            if ModdedVehicles[tostring(veh)] == nil and not IsThisModelABicycle(GetEntityModel(veh)) then
                ...
                ModdedVehicles[tostring(veh)] = {["fInitialDriveMaxFlatVel"] = fInitialDriveMaxFlatVel, ["fSteeringLock"] = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fSteeringLock'), ["fTractionLossMult"] = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fTractionLossMult'), ["fLowSpeedTractionLossMult"] = GetVehicleHandlingFloat(veh, 'CHandlingData', 'fLowSpeedTractionLossMult')}
            else
                Citizen.Wait(1000)
            end
        else
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        rg.toiminnot.Notify("Estado desconhecido..", "error")
                    end
                else
                    rg.toiminnot.Notify("Veiculo invalido..", "error")
                end
            else
                rg.toiminnot.Notify("Não estas perto suficiente do veiculo..", "error")
            end
        else
            rg.toiminnot.Notify("Tens que entrar no veiculo primeiro..", "error")
        end
    else
        rg.toiminnot.Notify("Tens que estar fora do veiculo..", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
            ...
            rg.toiminnot.Notify("Não estas no lugar de condutor do veiculo..", "error")
            end
        else
            rg.toiminnot.Notify("Não estas num veiculo..", "error")
    end
end)
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        rg.toiminnot.Progressbar("repair_advanced", "KORJAAT..", lockpickTime, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            if part == "body" then
                                ...
                                TriggerServerEvent("rg:Server:RemoveItem", Config.RepairCost[part], needAmount)
                                TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items[Config.RepairCost[part]], "remove")
                            elseif part ~= "engine" then
                                TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                TriggerServerEvent("rg:Server:RemoveItem", Config.RepairCost[part], level)
                                TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items[Config.RepairCost[part]], "remove")
                            end
                        end, function()
                            openingDoor = false
                        ...
                        rg.toiminnot.Notify("Componente invalido.", "error")
                    end
                else
                    rg.toiminnot.Notify("Veiculo invalido..", "error")
                end
            else
                rg.toiminnot.Notify("Não estas perto do veiculo..", "error")
            end
        else
            rg.toiminnot.Notify("Tens que entrar no veiculo primeiro..", "error")
        end
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['companies']:IsEmployee(v)
    end
    return retval
end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {template = '<div class="chat-message normal"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>'.. Config.ValuesLabels["engine"] ..' (engine):</strong> {1} <br><strong>'.. Config.ValuesLabels["body"] ..' (body):</strong> {2} <br><strong>'.. Config.ValuesLabels["radiator"] ..' (radiator):</strong> {3} <br><strong>'.. Config.ValuesLabels["axle"] ..' (axle):</strong> {4}<br><strong>'.. Config.ValuesLabels["brakes"] ..' (brakes):</strong> {5}<br><strong>'.. Config.ValuesLabels["clutch"] ..' (clutch):</strong> {6}<br><strong>'.. Config.ValuesLabels["fuel"] ..' (fuel):</strong> {7}</div></div>', args = {'Voertuig Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..rg.datapankki.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..rg.datapankki.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..rg.datapankki.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..rg.datapankki.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..rg.datapankki.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..rg.datapankki.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..rg.datapankki.Items[Config.RepairCost["fuel"]]["label"]..")"}})
    end
end
