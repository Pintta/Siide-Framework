        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    rg.toiminnot.Notify("It seems that the fuses burned..", "error")
                end
            else
...
RegisterNetEvent('bankrobbery:client:SetStationStatus')
AddEventHandler('bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent("rg:Server:RemoveItem", "thermite", 1)
    TriggerEvent('inventory:client:ItemBox', rg.datapankki.Items["thermite"], "remove")
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(GetPlayerPed(-1))
    while time > 0 do 
        rg.toiminnot.Notify("Will start in " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    if currentStation ~= 0 then
        rg.toiminnot.Notify("The fuses have been broken", "success")
        TriggerServerEvent("bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        rg.toiminnot.Notify("The door is now open", "success")
        TriggerServerEvent('doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)
