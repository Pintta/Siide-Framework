...
RegisterServerEvent('bankrobbery:server:setBankState')
AddEventHandler('bankrobbery:server:setBankState', function(bankId, state)
    if bankId == "paleto" then
        if not robberyBusy then
            Config.BigBanks["paleto"]["isOpened"] = state
            TriggerClientEvent('bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('scoreboard:server:SetActivityBusy', "bankrobbery", true)
    ...
    elseif bankId == "pacific" then
        if not robberyBusy then
            ...
            TriggerClientEvent('bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('scoreboard:server:SetActivityBusy', "pacific", true)
            TriggerEvent('bankrobbery:server:setTimeout')
        end
    else
        if not robberyBusy then
...
RegisterServerEvent('bankrobbery:server:setLockerState')
AddEventHandler('bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterServerEvent('bankrobbery:server:recieveItem')
AddEventHandler('bankrobbery:server:recieveItem', function(type)
    local src = source
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items[item.item], "add")
                elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = math.random(4000, 6000)
                    }
                    ply.Functions.AddItem('markedbills', 1, false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items['markedbills'], "add")
                end
            else
                ply.Functions.AddItem('security_card_01', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items['security_card_01'], "add")
            end
        else
    ...
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items['security_card_02'], "add")
            end
        else
            ply.Functions.AddItem('weapon_vintagepistol', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items['weapon_vintagepistol'], "add")
        end
    elseif type == "pacific" then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items['cryptostick'], "add")
            end
        else
            ...
            if chance == odd then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items['weapon_microsmg'], "add")
            else
                ply.Functions.AddItem('weapon_minismg', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, rg.datapankki.Items['weapon_minismg'], "add")
            end
        end
    end
end)

rg.toiminnot.CreateCallback('bankrobbery:server:isRobberyActive', function(source, cb)
    cb(robberyBusy)
end)

rg.toiminnot.CreateCallback('bankrobbery:server:GetConfig', function(source, cb)
    cb(Config)
end)

RegisterServerEvent('bankrobbery:server:setTimeout')
    ...
    if not robberyBusy then
        if not timeOut then
            ...
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
                TriggerEvent('scoreboard:server:SetActivityBusy', "bankrobbery", false)
                TriggerEvent('scoreboard:server:SetActivityBusy', "pacific", false)
...
RegisterServerEvent('bankrobbery:server:SetSmallbankTimeout')
...
RegisterServerEvent('bankrobbery:server:callCops')
AddEventHandler('bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    local cameraId = 4
    ...
    TriggerClientEvent("bankrobbery:client:robberyCall", -1, type, bank, streetLabel, coords)
    TriggerClientEvent("phone:client:addPoliceAlert", -1, alertData)
end)

RegisterServerEvent('bankrobbery:server:SetStationStatus')
AddEventHandler('bankrobbery:server:SetStationStatus', function(key, isHit)
...
function CheckStationHits()
    ...
    if Config.PowerStations[11].hit and Config.PowerStations[1].hit and Config.PowerStations[2].hit then
        ...
        TriggerClientEvent("bankrobbery:client:BankSecurity", 1, false)
        TriggerClientEvent("police:client:SetCamera", -1, 22, false)
        TriggerClientEvent("bankrobbery:client:BankSecurity", 2, false)
    end
    if Config.PowerStations[8].hit and Config.PowerStations[4].hit and Config.PowerStations[5].hit and Config.PowerStations[6].hit then
        ...
        TriggerClientEvent("bankrobbery:client:BankSecurity", 3, false)
    end
    if Config.PowerStations[12].hit and Config.PowerStations[13].hit then
        TriggerClientEvent("police:client:SetCamera", -1, 24, false)
        TriggerClientEvent("bankrobbery:client:BankSecurity", 4, false)
        TriggerClientEvent("police:client:SetCamera", -1, 25, false)
        TriggerClientEvent("bankrobbery:client:BankSecurity", 5, false)
    end
end
...
rg.toiminnot.CreateUseableItem("thermite", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
	if Player.Functions.GetItemByName('lighter') ~= nil then
        TriggerClientEvent("thermite:UseThermite", source)
    else
        TriggerClientEvent('rg:Ilmoitus', source, "You're missing something to light the termite..", "error")
    end
end)

rg.toiminnot.CreateUseableItem("security_card_01", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
...
rg.toiminnot.CreateUseableItem("security_card_02", function(source, item)
    local Player = rg.toiminnot.GetPlayer(source)
