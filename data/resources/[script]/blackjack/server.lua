		...
		while true do Wait(0)
			if players[index] and #players[index] ~= 0 then
				...
				if #players[index] == 0 then
					DebugPrint("Panostus on suljettu, "..index..", ei enään pakosia kiitos..")
				else
					for i,v in pairs(players[index]) do
						if v.bet < 1 then
							v.player_in = false
					...
					if ArePlayersStillIn(players[index]) then
						...
						for i,v in pairs(currentPlayers) do
							...
							if v.player_in == true and (handValue(v.hand) > handValue(dealerHand) or handValue(dealerHand) > 21) then
								if v.splitHand then
									if handValue(v.splitHand) > handValue(dealerHand) or handValue(dealerHand) > 21 then
										TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "good")
										DebugPrint("TABLE "..index..": "..GetPlayerName(v.player):upper().." VOITTO")
										v.player_in = false
									end
								end
								TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "good")
							...
							if v.player_in == true and handValue(v.hand) == handValue(dealerHand) then
								if v.splitHand then
									if handValue(v.splitHand) == handValue(dealerHand) then
										TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "impartial")
										DebugPrint("TABLE "..index..": "..GetPlayerName(v.player):upper().." TASAPELI")
										v.player_in = false
									end
								end
								TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "impartial")
							...
							if v.player_in == true and handValue(v.hand) < handValue(dealerHand) and handValue(dealerHand) <= 21 then
								if v.splitHand then
									if handValue(v.splitHand) < handValue(dealerHand) and handValue(dealerHand) <= 21 then
										TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "bad")
										DebugPrint("TABLE "..index..": "..GetPlayerName(v.player):upper().." HÄVIÖ")
										v.player_in = false
									end
								end
								TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "bad")
