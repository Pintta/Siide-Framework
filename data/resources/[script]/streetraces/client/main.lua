...
RegisterNetEvent('streetraces:StartRace')
AddEventHandler('streetraces:StartRace', function(race)
...
RegisterNetEvent('streetraces:RaceDone')
AddEventHandler('streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        rg.toiminnot.Notify(" The race endded. "..winner.. "!")
    end
end)

RegisterNetEvent('streetraces:StopRace')
AddEventHandler('streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('streetraces:CreateRace')
AddEventHandler('streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(GetPlayerPed(-1), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            rg.toiminnot.Notify("Race gemaakt voor $"..amount..",-!", "success")
        else
            rg.toiminnot.Notify("end position to close!", "error")
        end
    else
        rg.toiminnot.Notify("You have to place a marker!", "error")
    end
end)

RegisterNetEvent('streetraces:SetRace')
AddEventHandler('streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('streetraces:SetRaceId')
AddEventHandler('streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        rg.toiminnot.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    rg.toiminnot.Notify("GOOOOOOOOO!!!")
end
