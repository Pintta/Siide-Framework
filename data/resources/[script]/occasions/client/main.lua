            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        rg.toiminnot.HuutoKaiku('occasions:server:getVehicles', function(vehicles)
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, rg.datapankki.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Preço: $'..Config.OccasionSlots[i]["price"]..',-')
                                if Config.OccasionSlots[i]["owner"] == rg.toiminnot.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    rg.toiminnot.HuutoKaiku('occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
                                ...
                                if IsDisabledControlJustPressed(0, Keys["7"]) then
                                    isConfirming = false
                                    currentVehicle = i
                                    TriggerServerEvent("occasions:server:ReturnVehicle", Config.OccasionSlots[i])
                                end
                ...
                if sellDist <= 13.0 and IsPedInAnyVehicle(ped) then 
                    ...
                    if sellDist <= 3.5 and IsPedInAnyVehicle(ped) then
                        ...
                        if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            rg.toiminnot.HuutoKaiku('garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    openSellContract(true)
                                else
                                    rg.toiminnot.Notify('Does this vehicle belong to you?', 'error', 3500)
                                end
                            end, VehiclePlate)
                        end
                    end
...
function spawnOccasionsVehicles(vehicles)
    ...
    if vehicles ~= nil then
        for i = 1, #vehicles, 1 do
            ...
            rg.toiminnot.SetVehicleProperties(oSlot[i]["occasionid"], json.decode(oSlot[i]["mods"]))
...
function despawnOccasionsVehicles()
    ...
    for i = 1, #Config.OccasionSlots, 1 do
        ...
        if oldVehicle ~= 0 then
            rg.toiminnot.DeleteVehicle(oldVehicle)
        end
    end
end
...
function openSellContract(bool)
    ...
    SendNUIMessage({
        action = "sellVehicle",
        pData = rg.toiminnot.GetPlayerData(),
...
RegisterNUICallback('error', function(data)
    rg.toiminnot.Notify(data.message, 'error')
end)

RegisterNUICallback('buyVehicle', function()
    local vehData = Config.OccasionSlots[currentVehicle]
    TriggerServerEvent('occasions:server:buyVehicle', vehData)
end)

DoScreenFadeIn(250)

RegisterNetEvent('occasions:client:BuyFinished')
AddEventHandler('occasions:client:BuyFinished', function(model, plate, mods)
    ...
    rg.toiminnot.HaeAjoneuvo(model, function(veh)
        ...
        rg.toiminnot.Notify("Purchased vehicle", "success", 2500)
        TriggerEvent("vehiclekeys:client:AsetaOmistaja", plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        rg.toiminnot.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('occasions:client:ReturnOwnedVehicle')
AddEventHandler('occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    rg.toiminnot.HaeAjoneuvo(vehdata.model, function(veh)
        SetVehicleNumberPlateText(veh, vehdata.plate)
        SetEntityHeading(veh, Config.BuyVehicle.h)
        TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
        exports['LegacyFuel']:SetFuel(veh, 100)
        rg.toiminnot.Notify("You removed your vehicle from the stand ..")
        TriggerEvent("vehiclekeys:client:AsetaOmistaja", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        rg.toiminnot.SetVehicleProperties(veh, vehmods)
    end, Config.BuyVehicle, true)
...
RegisterNUICallback('sellVehicle', function(data)
    ...
    vehicleData.model = rg.datapankki.VehicleModels[GetEntityModel(vehicleData.ent)]["model"]
    vehicleData.plate = GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1)))
    vehicleData.mods = rg.toiminnot.GetVehicleProperties(vehicleData.ent)
...
function sellVehicleWait(price)
    ...
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    rg.toiminnot.Notify('Your car is on sale for $'..price..',-', 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

RegisterNetEvent('occasion:client:refreshVehicles')
AddEventHandler('occasion:client:refreshVehicles', function()
    if inRange then
        rg.toiminnot.HuutoKaiku('occasions:server:getVehicles', function(vehicles)
            occasionVehicles = vehicles
            despawnOccasionsVehicles()
...
Citizen.CreateThread(function()
    ...
    SetBlipColour(OccasionBlip, 81)

    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("Käytetyt")
