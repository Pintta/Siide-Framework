...
RegisterNetEvent('diving:client:NewLocations')
AddEventHandler('diving:client:NewLocations', function()
...
RegisterNetEvent('diving:client:SetDivingLocation')
AddEventHandler('diving:client:SetDivingLocation', function(DivingLocation)
    ...
    Citizen.CreateThread(function()
        RadiusBlip = AddBlipForRadius(rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.z, 100.0)
        
        SetBlipRotation(RadiusBlip, 0)
        SetBlipColour(RadiusBlip, 81)
        
        CurrentDivingLocation.Blip.Radius = RadiusBlip

        LabelBlip = AddBlipForCoord(rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.z)

        SetBlipSprite (LabelBlip, 597)
        SetBlipDisplay(LabelBlip, 4)
        SetBlipScale  (LabelBlip, 0.7)
        SetBlipColour(LabelBlip, 81)
        SetBlipAsShortRange(LabelBlip, true)

        BeginTextCommandSetBlipName('STRING')
        AddTextComponentSubstringPlayerName('Sukellusalue')
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentDivingLocation.Area ~= 0 then
            local AreaDistance = GetDistanceBetweenCoords(Pos, rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, rgDiving.Locations[CurrentDivingLocation.Area].coords.Area.z)
            ...
            if inRange then
                for cur, CoralLocation in pairs(rgDiving.Locations[CurrentDivingLocation.Area].coords.Coral) do
                            ...
                            if not CoralLocation.PickedUp then
                                ...
                                if CoralDistance <= 1.5 then
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        ...
                                        rg.toiminnot.Progressbar("take_coral", "Collecting coral..", times * 1000, false, true, {
...
function TakeCoral(coral)
    rgDiving.Locations[CurrentDivingLocation.Area].coords.Coral[coral].PickedUp = true
    TriggerServerEvent('diving:server:TakeCoral', CurrentDivingLocation.Area, coral, true)
end

RegisterNetEvent('diving:client:UpdateCoral')
AddEventHandler('diving:client:UpdateCoral', function(Area, Coral, Bool)
...
RegisterNetEvent('diving:server:CallCops')
AddEventHandler('diving:server:CallCops', function(Coords, msg)
    ...
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("HÄTÄKESKUS: Hätäsoitto")
...
RegisterNetEvent('diving:client:UseGear')
AddEventHandler('diving:client:UseGear', function(bool)
    if bool then
        GearAnim()
        rg.toiminnot.Progressbar("equip_gear", "Putting on wet suit..", 5000, false, true, {}, {}, {}, {}, function()
            DeleteGear()
            ...
            TriggerServerEvent('diving:server:RemoveGear')
        ...
        if currentGear.enabled then
            GearAnim()
            rg.toiminnot.Progressbar("remove_gear", "Removing wet suit..", 5000, false, true, {}, {}, {}, {}, function()
                DeleteGear()

                SetEnableScuba(GetPlayerPed(-1), false)
                SetPedMaxTimeUnderwater(GetPlayerPed(-1), 1.00)
                currentGear.enabled = false
                TriggerServerEvent('diving:server:GiveBackGear')
                ClearPedTasks(GetPlayerPed(-1))
                rg.toiminnot.Notify('You took your wetsuit off')
            end)
        else
            rg.toiminnot.Notify('You are not wearing a diving gear..', 'error')
        end
    end
end)
