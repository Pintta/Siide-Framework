...
function checkFunction()
  for i, modelConfig in pairs(globalConfig.props) do
    ...
    if DoesEntityExist(table) then
      ...
      TriggerServerEvent('kadenvaanto:check_sv', position)
      break
    end
  end
end

RegisterNetEvent('kadenvaanto:updategrade_cl')
AddEventHandler('kadenvaanto:updategrade_cl', function(gradeUpValue)

  grade = gradeUpValue

end)

RegisterNetEvent('kadenvaanto:start_cl')
AddEventHandler('kadenvaanto:start_cl', function()
  started = true
  if place == 1 then
    ...
    while grade >= 0.10 and grade <= 0.90 do
      ...
      if IsControlPressed(0, 38) then
        TriggerServerEvent('kadenvaanto:updategrade_sv', 0.015) 
        SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling", "sweep_a", grade)
    ...
    Wait(4000)
    TriggerServerEvent('kadenvaanto:disband_sv', GetEntityCoords(PlayerPedId()))
    return
...
RegisterNetEvent('kadenvaanto:check_cl')
AddEventHandler('kadenvaanto:check_cl', function(args)
    local table = 0
    if args == 'place1' then
      place = 1
...
RegisterNetEvent('kadenvaanto:reset_cl')
AddEventHandler('kadenvaanto:reset_cl', function()
    for i, modelConfig in pairs(globalConfig.props) do
