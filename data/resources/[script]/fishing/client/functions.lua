TryToFish = function()
    rg.toiminnot.HuutoKaiku('fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return rg.toiminnot.Notify("You can't be swimming and fishing at the same time.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return rg.toiminnot.Notify("You need to exit your vehicle to start fishing.", "error") end 
        if count ~= nil then
            if count == 0 then
                rg.toiminnot.Notify("You need both a fishing rod and bait to fish.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    rg.toiminnot.Notify("You need to aim towards the water to fish", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        rg.toiminnot.Notify('You don\'t have any bait!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            rg.toiminnot.Notify("You need to cast the bait.", "primary")
    ...
    rg.toiminnot.Notify("Waiting for a fish to bite...", "success", "10000")
    TriggerServerEvent('rg:Server:RemoveItem', "fishingbait", 1)
	TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items["fishingbait"], "remove")
    ...
    if caughtFish then
        TriggerServerEvent("fishing:receiveFish", castLocation, function(received) end)
        TriggerServerEvent('hud:Server:RelieveStress', 1)
    else
        rg.toiminnot.Notify("The fish got loose.", "error")
    end
...
IsInWater = function()
    ...
    rg.toiminnot.Notify("Checking fishing location...", "success", "3000")
...
SellFish = function()
    rg.toiminnot.HuutoKaiku('fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("fishing:sellFish", function(sold, fishesSold) end)
    end)
end
...
function HasFishingBait()
    local rtval = false
    rg.toiminnot.HuutoKaiku('rg:HasItem', function(amount)
        if amount then
            rtval = true
        end
    end, "fishingbait")
