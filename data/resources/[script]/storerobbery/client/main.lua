...
Citizen.CreateThread(function()
    Wait(1000)
    if rg.toiminnot.GetPlayerData().job ~= nil and next(rg.toiminnot.GetPlayerData().job) then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        rg.toiminnot.HuutoKaiku('storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    rg.toiminnot.Notify("Não existe policia suficiente", "error")
                                end
                            end
                        else
                ...
                if usingAdvanced then
                    ...
                    rg.toiminnot.HuutoKaiku('rg:HasItem', function(result)
                        if result then
                            ...
                            rg.toiminnot.Notify("Precisas duma caixa de ferramentas..", "error")
                        end
                    end, "screwdriverset")
                end
...
function setupRegister()
    rg.toiminnot.HuutoKaiku('storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    rg.toiminnot.HuutoKaiku('storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        ...
        rg.toiminnot.Progressbar("search_register", "A esvaziar a registadora..", lockpickTime, false, true, {
...
RegisterNUICallback('fail', function()
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            ...
            TriggerEvent('inventory:client:ItemBox', rg.datapankki.Items["advancedlockpick"], "remove")
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent("rg:Server:RemoveItem", "lockpick", 1)
            TriggerEvent('inventory:client:ItemBox', rg.datapankki.Items["lockpick"], "remove")
        end
    end
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        rg.toiminnot.Notify("Cortas-te a tua mão com o lockpick..")
    end
    lockpick(false)
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    rg.toiminnot.HuutoKaiku('storerobbery:server:isCombinationRight', function(combination)
...
AddEventHandler('storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then
        ...
        AddTextComponentString("HÄTÄKESKUS: Kaupparyöstö käynnissä")
