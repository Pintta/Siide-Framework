...
RegisterNUICallback('save', function(data)
    rg.toiminnot.HuutoKaiku('tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = GetPlayerPed(-1)
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            rg.toiminnot.Notify('TunerJN v1.05: mudança de veículo!', 'error')
...
AddEventHandler('tunerchip:server:TuneStatus', function()
    ...
    rg.toiminnot.HuutoKaiku('tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    rg.toiminnot.Notify('TunerJN v1.05: Veículo reiniciado!', 'error')
end)
...
AddEventHandler('tunerchip:client:openChip', function()
    ...
    if inVehicle then
        rg.toiminnot.Progressbar("connect_laptop", "TunerJN está conectando..", 2000, false, true, {
            ...
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
            rg.toiminnot.Notify("Geannuleerd..", "error")
        end)
    else
        rg.toiminnot.Notify("Você não está em um veículo..", "error")
    end
end)
