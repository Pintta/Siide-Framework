...
AddEventHandler('rg:Client:OnPlayerLoaded', function()
    rg.toiminnot.HuutoKaiku('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            rg.toiminnot.Progressbar("use_nos", "A meter nitro...", 1000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function()
                TriggerEvent("inventory:client:ItemBox", rg.datapankki.Items['nitrous'], "remove")
                TriggerServerEvent("rg:Server:RemoveItem", 'nitrous', 1)
                local CurrentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
                local Plate = GetVehicleNumberPlateText(CurrentVehicle)
                TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
            end)
        else
            rg.toiminnot.Notify('Não estas dentro de um carro.', 'error')
        end
    else
        rg.toiminnot.Notify('Já tens equipado o nitro..', 'error')
    end
end)
