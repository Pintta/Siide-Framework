...
RegisterServerEvent("pawnshop:server:sellPawnItems")
AddEventHandler("pawnshop:server:sellPawnItems", function()
    local src = source
    local price = 0
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        TriggerClientEvent('rg:Ilmoitus', src, "You have sold your items")
    end
end)

RegisterServerEvent("pawnshop:server:sellHardwarePawnItems")
AddEventHandler("pawnshop:server:sellHardwarePawnItems", function()
    local src = source
    local price = 0
    local totalAmount = 0
    local Player = rg.toiminnot.GetPlayer(src)
...
RegisterServerEvent("pawnshop:server:getGoldBars")
AddEventHandler("pawnshop:server:getGoldBars", function()
    local src = source
    local Player = rg.toiminnot.GetPlayer(src)
    if GoldBarsAmount > 0 then
        if Player.Functions.AddItem("goldbar", GoldBarsAmount) then
            ...
            Config.MeltTime = 300
            TriggerClientEvent("pawnshop:client:SetTakeState", -1, false)
        else
            TriggerClientEvent('rg:Ilmoitus', src, "You dont have any space in your inventory", "error")
        end
    end
end)

RegisterServerEvent("pawnshop:server:sellGold")
AddEventHandler("pawnshop:server:sellGold", function()
    local src = source
    local Player = rg.toiminnot.GetPlayer(src)
    local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        TriggerClientEvent('rg:Ilmoitus', src, "You have sold your item's")
    end
end)

RegisterServerEvent("pawnshop:server:meltItems")
AddEventHandler("pawnshop:server:meltItems", function()
    local src = source
    local Player = rg.toiminnot.GetPlayer(src)
    local goldbars = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if MeltItems[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    if amount < 1 then
                        TriggerClientEvent('rg:Ilmoitus', src, "You dont have enough " .. Player.PlayerData.items[k].label, "error")
                    else
        ...
        if goldbars > 0 then
            ...
            TriggerClientEvent('pawnshop:client:startMelting', -1)
            ...
            Citizen.CreateThread(function()
                while Config.IsMelting do
                    ...
                    if Config.MeltTime <= 0 then
                        ...
                        Config.CanTake = true
                        Config.MeltTime = 300
                        TriggerClientEvent('pawnshop:client:SetTakeState', -1, true)
                    end
                    Citizen.Wait(1000)
                end
            end)
...
rg.toiminnot.CreateCallback('pawnshop:server:getSellPrice', function(source, cb)
    local retval = 0
...
rg.toiminnot.CreateCallback('pawnshop:melting:server:GetConfig', function(source, cb)
    cb(Config.IsMelting, Config.MeltTime, Config.CanTake)
end)

rg.toiminnot.CreateCallback('pawnshop:server:getSellHardwarePrice', function(source, cb)
    local retval = 0
    local amount = 0
    local Player = rg.toiminnot.GetPlayer(source)
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
...
rg.toiminnot.CreateCallback('pawnshop:server:hasGold', function(source, cb)
	local retval = false
    local Player = rg.toiminnot.GetPlayer(source)
