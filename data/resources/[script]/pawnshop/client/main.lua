				...
				if GetClockHours() >= 1 and GetClockHours() <= 4 then
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						...
						if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            rg.toiminnot.Progressbar("sell_pawn_items", "Selling stuff ..", math.random(15000, 25000), false, true, {}, {}, {}, {}, function()
                                ClearPedTasks(GetPlayerPed(-1))
								TriggerServerEvent("pawnshop:server:sellPawnItems")
								sellItemsSet = false
								sellPrice = 0
                            end, function()
								ClearPedTasks(GetPlayerPed(-1))
								rg.toiminnot.Notify("Canceled..", "error")
							end)
						end
					else
...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName("Panttilainaamo")
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.PawnHardwareLocation.x, Config.PawnHardwareLocation.y, Config.PawnHardwareLocation.z, true) < 1.5 then
				if GetClockHours() >= 9 and GetClockHours() <= 16 then
					...
					elseif sellHardwareItemsSet and sellHardwarePrice ~= 0 then
						...
						if IsControlJustReleased(0, Keys["E"]) then
							TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_STAND_IMPATIENT", 0, true)
                            rg.toiminnot.Progressbar("sell_pawn_items", "Selling ..", math.random(15000, 25000), false, true, {}, {}, {}, {}, function()
                                ClearPedTasks(GetPlayerPed(-1))
								TriggerServerEvent("pawnshop:server:sellHardwarePawnItems")
...
function GetSellingPrice()
	local price = 0
	rg.toiminnot.HuutoKaiku('pawnshop:server:getSellPrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price
...
function GetSellingHardwarePrice()
	local price = 0
	rg.toiminnot.HuutoKaiku('pawnshop:server:getSellHardwarePrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price
