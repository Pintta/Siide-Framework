...
AddEventHandler("rg:Client:OnPlayerLoaded", function()
    rg.toiminnot.HuutoKaiku('pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("pawnshop:server:getGoldBars")
                        end
                    else
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then 
                            ...
                            ScrapAnim(1000)
                            rg.toiminnot.Progressbar("drop_golden_stuff", "Picking up items..", 1000, false, true, {
                                ...
                                if not Config.IsMelting then
                                    ...
                                    TriggerServerEvent("pawnshop:server:meltItems")
                                end
                            end)
                        end
                    end
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            rg.toiminnot.Progressbar("sell_gold", "Sell gold..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                TriggerServerEvent('pawnshop:server:sellGold')
                            end, function()
                                openingDoor = false
                                ClearPedTasks(GetPlayerPed(-1))
...
function HasPlayerGold()
	local retval = false
	rg.toiminnot.HuutoKaiku('pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('pawnshop:client:startMelting')
AddEventHandler('pawnshop:client:startMelting', function()
    if not Config.IsMelting then
...
RegisterNetEvent('pawnshop:client:SetTakeState')
AddEventHandler('pawnshop:client:SetTakeState', function(state)
