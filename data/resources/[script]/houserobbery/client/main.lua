...
AddEventHandler('rg:Client:OnPlayerLoaded', function()
    rg.toiminnot.HuutoKaiku('houserobbery:server:GetHouseConfig', function(HouseConfig)
...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    requiredItems = {
        [1] = {name = rg.datapankki.Items["lockpick"]["name"], image = rg.datapankki.Items["lockpick"]["image"]},
        [2] = {name = rg.datapankki.Items["screwdriverset"]["name"], image = rg.datapankki.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
...
function enterRobberyHouse(house)
    TriggerServerEvent("interact-sound:PlayOnSource", "houses_door_open", 0.25)
    openHouseAnim()
    Citizen.Wait(250)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['interior']:CreateTier1HouseFurnished(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('houserobbery:client:ResetHouseState')
AddEventHandler('houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('houserobbery:client:enterHouse')
AddEventHandler('houserobbery:client:enterHouse', function(house)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        ...
                        rg.toiminnot.Notify('The door is already open..', 'error', 3500)
                    end
                else
                    rg.toiminnot.Notify('There are not enough police..', 'error', 3500)
                end
            end
        else
            rg.toiminnot.HuutoKaiku('rg:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        ...
                        rg.toiminnot.Notify('It looks like you lack a toolbox for this...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        ...
        if closestPed ~= nil then
            ...
            local gender = "Male"
            if rg.toiminnot.GetPlayerData().charinfo.gender == 1 then
                gender = "Female"
            if rg.toiminnot.GetPlayerData().charinfo.gender == 2 then
                gender = "Binary"
            end
...
function lockpickFinish(success)
    if success then
        ...
        rg.toiminnot.Notify('You got it!', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = rg.datapankki.Items["advancedlockpick"]
            if math.random(1, 100) < 20 then
                TriggerServerEvent("rg:Server:RemoveItem", "advancedlockpick", 1)
                TriggerEvent('inventory:client:ItemBox', itemInfo, "remove")
            end
        else
            local itemInfo = rg.datapankki.Items["lockpick"]
            if math.random(1, 100) < 40 then
                TriggerServerEvent("rg:Server:RemoveItem", "lockpick", 1)
                TriggerEvent('inventory:client:ItemBox', itemInfo, "remove")
            end
        end
        
        rg.toiminnot.Notify('Falhaste..', 'error', 2500)
    end
end

RegisterNetEvent('houserobbery:client:setHouseState')
AddEventHandler('houserobbery:client:setHouseState', function(house, state)
...
function searchCabin(cabin)
    ...
    Skillbar.Start({
        ...
        ClearPedTasks(GetPlayerPed(-1))
        TriggerServerEvent('houserobbery:server:SetBusyState', cabin, currentHouse, false)
        rg.toiminnot.Notify("Process canceled..", "error")
        SucceededAttempts = 0
        FreezeEntityPosition(ped, false)
        SetTimeout(500, function()
            IsLockpicking = false
...
RegisterNetEvent('houserobbery:client:setCabinState')
AddEventHandler('houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('houserobbery:client:SetBusyState')
AddEventHandler('houserobbery:client:SetBusyState', function(cabin, house, bool)
