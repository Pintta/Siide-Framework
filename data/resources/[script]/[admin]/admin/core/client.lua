...
local PlayerBlips = {}
RGraamattu = {}
RGraamattu.Functions = {}
in_noclip_mode = false

RGraamattu.Functions.DrawText3D = function(x, y, z, text, lines)
    if lines == nil then
        lines = 1
    end
...
AddEventHandler('rg:Client:OnPlayerLoaded', function()
    TriggerServerEvent("admin:server:loadPermissions")
end)
...
AddEventHandler('admin:client:openMenu', function(group, dealers)
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('selfOptions') then
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                ...
                TriggerServerEvent("admin:server:togglePlayerNoclip", targetId)
            end
            if WarMenu.Button('Revive') then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                TriggerServerEvent('admin:server:revivePlayer', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('serverMan') then
            ...
            if WarMenu.ComboBox('Server time', times, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent("weathersync:server:setTime", time.hour, time.minute)
            end
            WarMenu.Display()
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            if WarMenu.MenuButton('Kill', currentPlayer) then
                TriggerServerEvent("admin:server:killPlayer", GetPlayerServerId(currentPlayer))
            end
            if WarMenu.MenuButton('Revive', currentPlayer) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent('admin:server:revivePlayer', target)
            end
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("admin:server:togglePlayerNoclip", target)
            end
            if WarMenu.CheckBox("Freeze", isFreeze, function(checked) isFreeze = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("admin:server:Freeze", target, isFreeze)
            end
            ...
            if WarMenu.MenuButton("Give Clothing Menu", currentPlayer) then
                ...
                TriggerServerEvent('admin:server:OpenSkinMenu', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('teleportOptions') then
            ...
            if WarMenu.MenuButton('Bring', currentPlayer) then
                ...
                TriggerServerEvent('admin:server:bringTp', GetPlayerServerId(currentPlayer), plyCoords)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent('admin:server:setPermissions', target, group)

                rg.toiminnot.Notify('You changed '..GetPlayerName(currentPlayer)..'\'s group to '..group.label)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('adminOptions') then
            if WarMenu.ComboBox('Ban length', bans, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                if reason ~= nil and reason ~= "" and time ~= 0 then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("admin:server:banPlayer", target, time, reason)
                end
            end
            if WarMenu.MenuButton('Kick', currentPlayer) then
                ...
                if reason ~= nil and reason ~= "" then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("admin:server:kickPlayer", target, reason)
                end
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                    TriggerServerEvent('weathersync:server:setWeather', AvailableWeatherTypes[k].weather)
                    rg.toiminnot.Notify('Weather changed to: '..AvailableWeatherTypes[k].label)
                end
            end
            WarMenu.Display()
        ...
        elseif WarMenu.IsMenuOpened('createDealer') then
            if WarMenu.ComboBox('Hora minima', times, currentMinTimeIndex, selectedMinTimeIndex, function(currentIndex, selectedIndex)
                ...
                rg.toiminnot.Notify('Hora confirmada!', 'success')
            end
    ...
    while true do
        if showNames then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(GetPlayerPed(-1)), 5.0)) do
                ...
                RGraamattu.Functions.DrawText3D(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z + 1.0, '['..PlayerId..'] '..PlayerName)
            end
        else
            Citizen.Wait(1000)
        end
	...
	while true do
        ...
        if deleteLazer then
            ...
            if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                ...
                RGraamattu.Functions.DrawText3D(entityCoord.x, entityCoord.y, entityCoord.z, "Obj: " .. entity .. " Model: " .. GetEntityModel(entity).. " \nPress [~g~E~s~] to delete the object!", 2)
...
RegisterNetEvent('admin:client:bringTp')
AddEventHandler('admin:client:bringTp', function(coords)
    local ped = GetPlayerPed(-1)
    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('admin:client:Freeze')
AddEventHandler('admin:client:Freeze', function(toggle)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('admin:client:SendReport')
AddEventHandler('admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('admin:server:SendReport', name, src, msg)
end)

RegisterNetEvent('admin:client:SendStaffChat')
AddEventHandler('admin:client:SendStaffChat', function(name, msg)
    TriggerServerEvent('admin:server:StaffChatMessage', name, msg)
end)

RegisterNetEvent('admin:client:SaveCar')
AddEventHandler('admin:client:SaveCar', function()
    local ped = GetPlayerPed(-1)
    local veh = GetVehiclePedIsIn(ped)
    if veh ~= nil and veh ~= 0 then
        local plate = GetVehicleNumberPlateText(veh)
        local props = rg.toiminnot.GetVehicleProperties(veh)
        local hash = props.model
        if rg.datapankki.VehicleModels[hash] ~= nil and next(rg.datapankki.VehicleModels[hash]) ~= nil then
            TriggerServerEvent('admin:server:SaveCar', props, rg.datapankki.VehicleModels[hash], GetHashKey(veh), plate)
        else
            rg.toiminnot.Notify('You cant store this vehicle in your garage..', 'error')
        end
    else
        rg.toiminnot.Notify('You are not in a vehicle..', 'error')
    end
end)
...
RegisterNetEvent('admin:client:SetModel')
AddEventHandler('admin:client:SetModel', function(skin)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('admin:client:SetSpeed')
AddEventHandler('admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('weapons:client:SetWeaponAmmoManual')
AddEventHandler('weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = GetPlayerPed(-1)
    if weapon ~= "current" then
        local weapon = weapon:upper()
        SetPedAmmo(ped, GetHashKey(weapon), ammo)
        rg.toiminnot.Notify('+'..ammo..' Ammo for the '..rg.datapankki.Weapons[GetHashKey(weapon)]["label"], 'success')
    else
        local weapon = GetSelectedPedWeapon(ped)
        if weapon ~= nil then
            SetPedAmmo(ped, weapon, ammo)
            rg.toiminnot.Notify('+'..ammo..' Ammo for the '..rg.datapankki.Weapons[weapon]["label"], 'success')
        else
            rg.toiminnot.Notify('You dont have a weapon in your hands..', 'error')
        end
    end
end)

RegisterNetEvent('admin:client:GiveNuiFocus')
AddEventHandler('admin:client:GiveNuiFocus', function(focus, mouse)
