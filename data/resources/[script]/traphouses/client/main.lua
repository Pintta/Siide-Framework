...
Citizen.CreateThread(function()
    ...
    if rg.toiminnot.GetPlayerData() ~= nil then
        ...
        rg.toiminnot.HuutoKaiku('traphouses:server:GetTraphousesData', function(trappies)
...
RegisterNetEvent('traphouses:client:EnterTraphouse')
AddEventHandler('traphouses:client:EnterTraphouse', function(code)
    if ClosestTraphouse ~= nil then
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        rg.toiminnot.Notify('This code is not valide..', 'error')
    end
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('traphouses:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent('traphouses:server:TakeoverHouse', CurrentTraphouse)
                            end
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, Keys["G"]) then
                                    rg.toiminnot.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, Keys["H"]) then
                                local TraphouseInventory = {}
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent("traphouses:server:TakeMoney", CurrentTraphouse)
                            end
                        end
                    end
                end
...
RegisterNetEvent('traphouses:client:TakeoverHouse')
AddEventHandler('traphouses:client:TakeoverHouse', function(TraphouseId)
    local ped = GetPlayerPed(-1)

    rg.toiminnot.Progressbar("takeover_traphouse", "Overtaking traphouse", math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function()
        TriggerServerEvent('traphouses:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        rg.toiminnot.Notify("Overtaking canceled..", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            rg.toiminnot.Notify(CitizenId..' is added to the traphouse!')
        else
            rg.toiminnot.Notify(CitizenId..' This person does already have the keys!')
        end
    else
        rg.toiminnot.Notify('You can max give 6 other people acces to the trap house!')
    end
    IsKeyHolder = HasKey(CitizenId)
    IsHouseOwner = IsOwner(CitizenId)
end

RegisterNetEvent('traphouses:client:SyncData')
AddEventHandler('traphouses:client:SyncData', function(k, data)
