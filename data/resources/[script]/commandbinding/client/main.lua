...
RegisterNetEvent('commandbinding:client:openUI')
AddEventHandler('commandbinding:client:openUI', function()
    openBindingMenu()
end)
        ...
        if isLoggedIn then
            for k, v in pairs(availableKeys) do
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    local keyMeta = rg.toiminnot.GetPlayerData().metadata["commandbinds"]
                    local args = {}
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            TriggerServerEvent('rg:CallCommand', keyMeta[v]["command"], args)
                            keyPressed = true
                        else
                            rg.toiminnot.Notify('You do not have anything bound to ['..v..'] , /binds for binds a command', 'primary', 4000)
                        end
                    else
                        rg.toiminnot.Notify('Indicates a command, /binds for binde', 'primary', 4000)
                    end
                end
            end
...
RegisterNUICallback('save', function(data)
    ...
    rg.toiminnot.Notify('Saved your binds!', 'success')
